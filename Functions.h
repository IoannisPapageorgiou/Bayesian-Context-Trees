...
void log_loss(vector <short> xn, int train_size); // computes the log-loss incurred by prediction and writes it in file "log_loss.txt"
...
long double ctw(tree& T) {                   // algorithm takes improper tree and finds prior-predictive likelihood
	...
	cout <<endl<< "Prior-predictive likelihood is " << pow(2.0, T[0][0]->lw) << " and has log " << T[0][0]->lw << endl ;
